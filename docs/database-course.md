...
### Текущий workflow после T5

Migration — зафиксированная SQL-инструкция перехода базы из одной версии в другую. Начальная миграция уже хранится в `prisma/migrations`; обычный `npm run dev` применяет сохранённую историю через `npm run db:deploy`.

Типичный цикл:

1. изменить `prisma/schema.prisma`;
2. на dev-базе выполнить `npm run prisma:migrate -- --name имя_изменения`;
3. проверить SQL и сохранить новую миграцию вместе со schema в Git;
4. выполнить `npm run check` и перезапустить `npm run dev`. TypeScript-код API перезапускается автоматически.

### Существующая база и общее окружение

Для базы, созданной до миграций, сначала остановите API, сохраните backup и зарегистрируйте baseline после обновления до T4:

```powershell
npm run db:baseline
```

На сервере применяется уже существующая история:

```powershell
npm run db:deploy
```

`db:baseline` проверяет схему и счётчики истории до регистрации, данные не переписывает. `migrate dev` требует отдельную shadow database или право её создания; на общем окружении он не запускается. `db push` остаётся только для одноразовых экспериментов на отдельной БД. Пошаговый переход, backup/restore и ограничения — в [backend guide](backend-guide.md#изменение-модели-данных).

...
5. Использовать tests first для pet rules, storage, backend logic и migrations.
6. Реализовывать маленькими шагами.
7. Проверить через tests/build или явно объяснить, почему проверка не нужна.
8. Сверить результат со spec и перечислить оставшиеся open questions.

## Практическое Правило

Если будущий участник спрашивает "откуда взялось это решение?", ответ должен находиться в `GAME_DESIGN.md`, focused spec под `docs/specs/` или тесте.
